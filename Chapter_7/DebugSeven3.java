...
class DebugSeven3
{
   ...
   { 
      ...
      quote = kb.nextLine();//fixed missing kb.
      System.out.print("Enter a character to search for >> ");
      charString = kb.next();//fixed 'kb.nextInt()' to 'kb.next()' because we need string
      searchChar = charString.charAt(0);
      System.out.println("index of('" + searchChar + "') is: " +
         quote.indexOf(searchChar));//fixed missing parentheses 'quote.indexOf(searchChar);'
      System.out.println("indexOf('a') is: " + quote.indexOf('a'));//fixed missing parentheses in 'quote.indexOf('a')'
      ...
      System.out.println("replace('e', '*') is: " +
         quote.replace('e', '*'));//fixed 'repplace'
   }
}
